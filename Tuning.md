...
**How a profile's `voicingTone` meets the listener's settings (3.10 beta 10):** the frontend reads 17 keys from a profile's `voicingTone` (the ones listed below; `creakiness`, `breathiness`, `jitter`, `shimmer`, `sharpness` and the chorus keys are listener settings and are not read from a profile). A block that leaves out `highShelfGainDb` plays 5.5 dB, where a voice without a block plays 4 dB. When a profile with its own block is active, its stored values are the base and every listener setting at its neutral position leaves them alone: the NVDA add-on, SAPI, Android and iOS all compose the same way (`src/voicingToneCompose.h`; `voicing_tone.py` for NVDA). Tilt, noise modulation, pitch-sync F1/B1, aspiration tilt and tremor add the setting's offset; speed quotient (relative to 2.0), formant sharpness (`cascadeBwScale`), head size (`f4FreqScale`) and the nasal scales multiply; moving a setting back to neutral restores the profile's value. Built-in voices and profiles without a block keep the plain slider mapping. The Linux speech-dispatcher module does the same with its `TGSB*` config settings (unset settings are neutral); its pre-emphasis and high-shelf settings apply to built-in voices only, since a profile supplies those. The phoneme editor's Save to Profile writes only the keys whose sliders moved away from what the source voice plays, and writes `highShelfGainDb` explicitly whenever adding or emptying a block would otherwise change the shelf the profile played, so an untouched save changes nothing.
...
- `aratoFramesPerSyllable` (26) is the program's frame density: its 12.8 ms frames at its own tempo. The pass sizes its virtual frame as the clause's duration divided by syllables × this value, so the frame count, and therefore the codes, stay the same at every rate and for our faster segments, as in the original where the tempo setting was never read by the intonation code. `aratoPitchByteHz` (2.441) is the chip's Hz per pitch-byte unit. `aratoReferencePitchHz` (103) is TALKHUN's own pitch, where the chip's hertz steps are the program's melody: the steps and the 40 Hz floor scale by base pitch / 103, so the melody keeps its intervals as the pitch slider moves instead of flattening at a high pitch and bottoming out at a low one (#136). At 103 Hz the output is the program's, frame for frame; 0 restores fixed hertz steps at any pitch.
